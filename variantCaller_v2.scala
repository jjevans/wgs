...
class variantCaller extends QScript with Logging {
	...
	def script() {
    	...
    	val sortprefix = conf(outBase) + ".sort" // sortSam prefix (add .bam to this)
    	val sort = conf(outBase) + ".sort.bam"	//sortSam
    	val dup = conf(outBase) + ".dup.bam"	//markDuplicates
    	val dupbai = conf(outBase) + ".dup.bai"	// bamIndex
    	...
    	add(createTarget(bamfile, bamindex, refFile, indelFile, intervalFile),
    		indelRealign(bamfile, intervalFile, refFile, indelFile, realignFile),
			samtoolsFixMate(realignFile, fixFile),
    		samtoolsIndex(fixFile, fixbaiFile),
    		baseRecal(fixFile, fixbaiFile, refFile, snpFile, covs, preFile),
    		printRead(fixFile, preFile, refFile, readFile),
    		samtoolsIndex(readFile, readbaiFile))
	    ...
	    add(callIndel(readFile, readbaiFile, refFile, conf(roi), indelVcf, indelMetric), //call indels
	    	callAtSite(readFile, readbaiFile, refFile, conf(roi), conf(dbsnp), "SNP", null, snpVcf, snpMetric)) // call snps
	    	//callAtSite(readFile, readbaiFile, refFile, conf(roi), conf(dbsnp), "BOTH", null, bothVcf, bothMetric)) // call indels and snps together
	    }
	...
	case class samtoolsSortSam(inBam: File, outBam: File, outPrefix: String) extends CommandLineFunction {
		// samtools sort
		val useMem: String = "16000000000"
		
		@Input(doc="BAM file to sort")
		val infile = inBam
		@Output(doc="Prefix to sorted BAM file to output")
		val outfile = outBam
		...
		this.isIntermediate = qscript.leaveFile
		this.analysisName = outfile + "-samtools_sort"
		this.jobName = outfile + "-samtools_sort"				  
	}
	...
	case class samtoolsMarkDup(inBam: File, outBam: File) extends CommandLineFunction {
		// samtools rmdup
		...
		this.isIntermediate = qscript.leaveFile
		this.analysisName = outfile + "-samtools_rmdup"
		this.jobName = outfile + "-samtools_rmdup"
	}
	...
	case class createTarget(inBam: File, inBai: File, ref: File, knownindel: File, outInterval: File) extends RealignerTargetCreator {
		// GATK RealignerTargetCreator
		
		@Input(doc="BAM file to find intervals with")
		val infile = inBam // need multiple input bams? 
		@Input(doc="BAM index file to find intervals with")
		val inbaifile = inBai
	...
	case class baseRecal(inBam: File, inBai: File, ref: File, knownsnp: File, covs: Seq[String], outBqsr: File) extends BaseRecalibrator {
	...
	case class callIndel(inBam: File, inBai: File, ref: File, intervalBed: File, outVcf: File, outMetric: File) extends UnifiedGenotyper {
	...
	case class callAtSite(inBam: File, inBai: File, ref: File, intervalBed: File, dbSnp: File, glmodel: String, outMode: String, outVcf: File, outMetric: File) extends UnifiedGenotyper {
		// GATK UnifiedGenotyper
