...
class variantCaller extends QScript with Logging {
	...
	val javaMem: String = "-Xmx" + memLimit.toString + "g"
	
	// native args for LSF/DRMAA in a Seq
	val jobGroup: String = "/slot6"
	...
	case class samtoolsSort(inBam: File, outBam: File, outPrefix: String) extends CommandLineFunction {
		// samtools sort
		// need both the BAM File and the outPrefix for samtools sort to use (and add .bam to)
		@Input(doc="BAM file to sort")
		val infile = inBam
		@Output(doc="sorted BAM file as output")
		val outfile = outBam
		...
		def commandLine = required("samtools") +
						  required("sort") + 
						  //required("-m",memLimit*1000000000) + //leave at default, put in GB, untested
						  required(infile) +
						  required(outPrefix)
